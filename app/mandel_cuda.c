...
//     CUDA support only ; the --fma option is ignored
    ...
    do {
        ...
        while (n < n_total) {
            ...
            if (get_dfloat_head(zreal_sqr2) + get_dfloat_head(zimag_sqr2) > ESCAPE_RADIUS_2) {
                ...
                return get_color2(colors, zreal_sqr2, zimag_sqr2, n + 3);
            }
...
// this function isn't used, inlined manually in mandelbrot2a()
    ...
    const int max_iters, const int width, const int height )
{
    ...
    temp[__umul24(width, pos_y) + pos_x] = mandel1b(colors, creal, cimag, max_iters);
  #else
    temp[__umul24(width, pos_y) + pos_x] = mandel1a(colors, creal, cimag, max_iters);
  #endif
}

__global__ void mandelbrot2a(
    const double min_x, const double min_y, const double step_x,
    ...
    const int height, const short aafactor, const double *offset )
{
    ...
    for (int i = 0; i < aaarea2; i += 2) {
        ...
        if (outside) {
            ...
            color = get_color2(colors, zreal_sqr2, zimag_sqr2, n + 3);
        }
        else
            color = INSIDE_COLOR1;
